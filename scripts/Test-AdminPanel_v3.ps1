# Comprehensive Admin Panel Test Script for PowerShell (Fixed Version - No Emojis)
...
function Test-ImageUploadMultipart {
    ...
    try {
        ...
        Write-Info "Vehicle ID: $($response.id)"
        if ($response.images) {
            Write-Success "Uploaded Images counts: $($response.images.Count)"
        }
...
$firebaseInitRes = Test-FirebaseInit
$jsonRes = Test-ExperienceSubmissionJSON
$multipartRes = Test-ImageUploadMultipart
