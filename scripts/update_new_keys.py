import os

# New keys provided by the user
new_private_key_id = "57f804e51996fb081bce390f8007467ffe3e6a9f"
new_private_key = """-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----"""

# Convert to single line with escaped newlines
formatted_key = new_private_key.replace('\n', '\\n')

env_path = '.env'
if os.path.exists(env_path):
    with open(env_path, 'r') as f:
        lines = f.readlines()
    
    with open(env_path, 'w') as f:
        for line in lines:
            if line.startswith('FIREBASE_ADMIN_PRIVATE_KEY_ID='):
                f.write(f'FIREBASE_ADMIN_PRIVATE_KEY_ID={new_private_key_id}\n')
            elif line.startswith('FIREBASE_ADMIN_PRIVATE_KEY='):
                f.write(f'FIREBASE_ADMIN_PRIVATE_KEY="{formatted_key}"\n')
            else:
                f.write(line)
    print("Updated .env successfully")
else:
    print(".env not found")
